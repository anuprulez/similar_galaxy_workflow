...
A neural network with convolutional layer is used to learn on the sequential data. The network has an embedding layer as the first layer and one convolutional and a dense layer as the hidden layers and an output dense layer. The parameters are learned by using Bayesian optimisation.
