...
A neural network with recurrent layers (GRU) are used to learn on the sequential data. The network has an embedding layer as the first layer and two GRU layers as hidden layers and an output dense layer. The parameters are learned by using Bayesian optimisation.
